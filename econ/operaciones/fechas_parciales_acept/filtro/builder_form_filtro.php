<?php
namespace econ\operaciones\fechas_parciales_acept\filtro;
...
class builder_form_filtro extends builder_formulario
{
	...
	function get_action() 
	{
		return kernel::vinculador()->crear('fechas_parciales_acept', 'index');
	}
	...
	function get_anios_academicos()
	{
		...
		//Esta operación sólo sirve a partir del año 2020, restirnjo los años anteriores
		$datos_2020 = Array();
		foreach($datos as $dato)
		{
			if ($dato['ANIO_ACADEMICO'] >= 2020) {
				$datos_2020[] = $dato;
			}
		}
		return guarani_form_elemento::armar_combo_opciones($datos_2020, '_ID_', 'ANIO_ACADEMICO', false, false, ucfirst(kernel::traductor()->trans('actas.filtro_seleccione')));
	}

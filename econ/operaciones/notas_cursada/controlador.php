...
class controlador extends \siu\operaciones\notas_cursada\controlador
{
    ...
    /**
     * @return siu\modelo\transacciones\carga_notas_cursada
     */
    function modelo()
    {
...
//										&& !($cierre_parcial && $fila_sesion['TIENE_NOTA'] == "S");
	...
	function accion__autocalcular()
	{
		...
		kernel::log()->add_debug('accion__autocalcular $acta', $acta);
		kernel::log()->add_debug('accion__autocalcular $term', $term);
		if (empty($term)) {
			$this->render_raw_json(array());
			return;
		}

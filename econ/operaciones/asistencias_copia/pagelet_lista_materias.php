...
class pagelet_lista_materias extends \siu\operaciones\asistencias\pagelet_lista_materias
{
    ...
    function get_lista_clases()
    {
        ...
        foreach ($dias_clases as $dc) 
        {
            ...
            $datos[$mat]['DIAS_CLASE'][$dia_semana]['CLASE'][$clase]['TIPO_CLASE'] = $dc['TIPO_CLASE'];
            ...
            $datos[$mat]['DIAS_CLASE'][$dia_semana]['CLASE'][$clase]['URL_PLANILLA'] = kernel::vinculador()->crear('asistencias', 'planilla', array('ID'=>$dc[catalogo::id])); //, 'MATERIA'=>$dc['MATERIA'], 'DIA_SEMANA'=>$dc['DIA_SEMANA'], 'HS_COMIENZO_CLASE'=>$dc['HS_COMIENZO_CLASE'])); //, 'TIPO_CLASE'=>$dc['TIPO_CLASE']));
            $datos[$mat]['DIAS_CLASE'][$dia_semana]['CLASE'][$clase]['URL_RESUMEN'] = kernel::vinculador()->crear('asistencias', 'resumen', array('ID'=>$dc[catalogo::id])); //, 'MATERIA'=>$dc['MATERIA'], 'DIA_SEMANA'=>$dc['DIA_SEMANA'], 'HS_COMIENZO_CLASE'=>$dc['HS_COMIENZO_CLASE'])); //, 'TIPO_CLASE'=>$dc['TIPO_CLASE']));
        }
        return $datos;
    }

    ...
    function accion__correr_evaluacion() 
    {
       ...
       // kernel::log()->add_debug('accion__confirmar_evaluacion parametros: ', $parametros);
        try
        {
			...
			foreach($comisiones as $comision)
            {
                ...
                if ($tiene_notas_cargadas) {
                    throw new error_guarani('La comisi�n '.$param['comision'].' tiene notas cragadas. ');
                }
                ...
                if (!$alta['success']) {
                    throw new error_guarani('La comisi�n '.$param['comision'].' no pudo ser modificada. ');
                }
            }
        ...
        catch (error_guarani $e)
        {
            ...
            $this->finalizar_request_con_notificaciones('No se puede modificar la fecha de la evaluaci�n. ', $e);
        }
    }
        ...
        try
        {
            ...
            foreach($datos_propuestos as $dato)
            {
                ...
                if (isset($eval_existente[0]) and isset($eval_existente[0]['FECHA_HORA'])) 
                {
                    ...
                    $alta = catalogo::consultar('cursos', 'alta_evaluacion_parcial', $param);
                    if (!$alta['success']) {
                        throw new error_guarani('La evaluaci�n para la comisi�n '.$dato['COMISION'].' no pudo ser creada. ');
                    }
                }
                else {
            ...
            $resultado['mensaje'] = 'Se cre� correctamente la evaluaci�n para la materia '.$parametros['materia'];
            $this->render_ajax('mensaje', $resultado);
        }
        catch (error_guarani $e)
        {
            kernel::db()->abortar_transaccion();
            $this->finalizar_request_con_notificaciones('No se pudo crear la evaluaci�n. ', $e);
        }
    }
		...
		if (!empty($anio_academico_hash))
        {
			...
			if (!empty($anio_academico) and !empty($periodo_hash))
			{
				...
				foreach ($resultado as $key=>$row) {
					//S�lo me quedo con los per�odos de evaluaciones parciales (1, 2, 3)

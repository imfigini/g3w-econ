...
class cursos 
{
    ...
    */
    function alta_propuesta_evaluacion_parcial($parametros)
    {
		...
		if (!empty($eval))
        {
			if ($eval['ESTADO'] != 'P') {//Si el estado es distinto de 'P' es que la instancia de evaluaci�n ya ha sido creada
				switch ($evaluacion)
				{
					case "'22'": $inst = '1� Parcial'; break;
					case "'23'": $inst = '2� Parcial'; break;
					case "'24'": $inst = 'Recuperatorio Global'; break;
					case "'14'": $inst = 'Integrador'; break;
				}
				throw new error_guarani("No se puede modificar la fecha propuesta dado que ya ha sido creada la instancia de evaluaci�n $inst para la comisi�n $comision. ");
			}
			else
			{
    ...
    */
    function get_estado_comision_fecha($parametros)
    {
		...
		//Sino, si la fecha corresponde a un d�a de cursada de la comisi�n devuelve 'C'
    ...
    */
    function alta_evaluacion_parcial($parametros)
    {
		...
		$comision = $parametros['comision'];
        $evaluacion = $parametros['evaluacion'];
		$escala = 3;	//Las instancias de evaluaci�n parcial siempre deben tener escala: 3 (Reales Regular)
	    $fecha_hora = $parametros['fecha_hora'];
        ...
        if ($result['mensaje'] == 'OK')
        {
			...
			$result['mensaje'] = "Se dio de alta correctamente la evaluaci�n $eval_descrip para la comisi�n $comision. ";
        }
        else
        {
			$result['success'] = -1;
